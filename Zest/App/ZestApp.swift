...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        DispatchQueue.main.async { [weak self] in
            guard let self, let window = NSApp.windows.first(where: { $0.level == .normal }) else { return }
            self.mainWindow = window
            self.setupWindowProxy(window)
        }
    ...
    @objc private func handleScreenLocked() {
        print("Screen locked — pausing Zest player")
        WallpaperManager.shared.isScreenLocked = true
        NotificationCenter.default.post(name: .wallpaperLockPause, object: nil)
    }

    @objc private func handleScreenUnlocked() {
        print("Screen unlocked — resuming Zest player")
        WallpaperManager.shared.isScreenLocked = false
        NotificationCenter.default.post(name: .wallpaperLockResume, object: nil)

        print("Killing WallpaperAgent on unlock to prepare for next lock screen...")
        WallpaperManager.shared.forceColdStartLockScreen()
    }
